...
class OpenAIProvider extends BaseProvider {
  ...
  async initializeClient() {
    ...
    if (!config.apiKey) {
      throw new Error(
        'OpenAI API key not configured. Run "aicommit setup" to configure.'
      );
    }
    ...
    try {
      ...
      const availableModels = models.data
        ...
        .sort((a, b) => {
          ...
          const priority =
            this.getModelPriority(a.id) - this.getModelPriority(b.id);
          return priority;
        })
        .map((model) => ({
          id: model.id,
  ...
  getModelDescription(modelId) {
    const descriptions = {
      ...
      'gpt-4-turbo-preview':
        'Latest GPT-4 with improved performance and lower cost',
  ...
  async validate(config) {
    ...
    if (config.model) {
      ...
      if (!modelExists) {
        console.warn(
          `Model ${config.model} not found in available models. Will use default.`
        );
      }
    }
