...
class ConfigValidator {
  ...
  validate(config) {
    ...
    for (const [key, rule] of Object.entries(this.validationRules)) {
      ...
      if (rule.type && valueType !== rule.type) {
        errors.push(
          `Invalid type for ${key}: expected ${rule.type}, got ${valueType} - ${rule.message}`
        );
        continue;
      }
      ...
      if (rule.enum && !rule.enum.includes(value)) {
        errors.push(
          `Invalid value for ${key}: "${value}" - ${rule.message}. Valid values: ${rule.enum.join(', ')}`
        );
        continue;
      }
      ...
      if (rule.type === 'number') {
        if (rule.min !== undefined && value < rule.min) {
          errors.push(
            `Value for ${key} is too low: ${value} - minimum is ${rule.min}. ${rule.message}`
          );
          continue;
        }
        if (rule.max !== undefined && value > rule.max) {
          errors.push(
            `Value for ${key} is too high: ${value} - maximum is ${rule.max}. ${rule.message}`
          );
          continue;
        }
      }
      ...
      if (rule.type === 'string') {
        if (rule.minLength && value.length < rule.minLength) {
          errors.push(
            `Value for ${key} is too short: ${value.length} characters - minimum is ${rule.minLength}. ${rule.message}`
          );
          continue;
        }
        if (rule.maxLength && value.length > rule.maxLength) {
          errors.push(
            `Value for ${key} is too long: ${value.length} characters - maximum is ${rule.maxLength}. ${rule.message}`
          );
          continue;
        }
      }
  ...
  validateProviderConfig(provider, config) {
    ...
    if (provider === 'groq') {
      if (!config.apiKey) {
        result.errors.push(
          'Groq provider requires an API key. Get one at https://console.groq.com/keys'
        );
      } else if (config.apiKey.length < 20) {
        result.errors.push(
          'Groq API key appears to be invalid (too short). Please verify your API key.'
        );
      }
    } else if (provider === 'ollama') {
      if (config.apiKey) {
        result.warnings.push(
          'Ollama provider does not require an API key. The API key setting will be ignored.'
        );
      }
    }
  ...
  generateSuggestions(result, config) {
    ...
    if (config.defaultProvider === 'groq' && !config.apiKey) {
      suggestions.push(
        'To get a Groq API key: Visit https://console.groq.com/keys and create a new key.'
      );
    }

    if (config.defaultProvider === 'ollama' && !this.isOllamaRunning()) {
      suggestions.push(
        'Make sure Ollama is running: Run "ollama serve" in a terminal before using the tool.'
      );
    }
